        ...
        var config = Array.prototype.reduce.call(
            ...
            function(values, el) {
                if (el.type === 'checkbox') {
                    ...
                    let int_or_float_val = parseInt(el.value);
                 let data_type = $$(el).data('data_type');
                     if (data_type === 'float'){
                         int_or_float_val = parseFloat(el.value);
                     }
                    if(!isNaN(int_or_float_val)) {
                        values[el.name] = int_or_float_val;
                    }
                    else {
